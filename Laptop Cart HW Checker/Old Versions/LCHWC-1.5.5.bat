...
echo.
echo This is the technical notes section. Please write any notes about the condition of the hardware, 
echo particularly the physical condition *after* being brought back. 
echo Please list who rented the laptop as well.
...
echo.
powershell -executionpolicy bypass -file LCHWCMailer.ps1 !serial! !datestr!
echo.
echo File successfully e-mailed to cedchelp@ucdenver.edu.
echo.
echo Sending file to Records folder on \\DATA store...
echo.
robocopy "C:\Users\cladmin\Desktop\LCHWC" "\\data\dept\CEAS\ITS\Labs\Laptop Cart Resources\Laptop_Hardware_Checker\Records" !datestr!-!serial!.txt /Z /ZB /R:5 /W:5 /TBD /NP /V /MT:16 >C:\Users\cladmin\Desktop\LCHWC\RobocopyResults.txt
echo File successfully sent to \\data\dept\CEAS\ITS\Labs\Laptop Cart Resources\Laptop_Hardware_Checker\Records.
echo.
set "reply=y"
set /p "reply=Perform Housekeeping Service(delete LCHWC Folder & its contents)? [y|n]: "
if /i not "!reply!" == "y" echo Script complete. & goto :EOF
echo.
@RD /S /Q "C:\Users\cladmin\Desktop\LCHWC"
...
echo.
echo This is the technical notes section. Please write any notes about the condition of the hardware, 
echo particularly the physical condition *before* being rented out. 
echo Please list who rented the laptop as well or wait until the post notes.
set /p prenotes="Technician Notes:"
>>"C:\Users\cladmin\Desktop\LCHWC\PreHardware.txt" echo "Technician Notes (Pre): !prenotes!" 
echo.
echo And finally making PreHardware.txt tamper resistant by locking it down to read-only...
