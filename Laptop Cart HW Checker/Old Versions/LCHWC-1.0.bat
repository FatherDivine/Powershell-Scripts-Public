...
::Checks if LCHWC folder already exists, and if not creates.
...
echo Technician Notes:
set /p PostNotes=""
cls
echo %PostNotes%>> C:\Users\cladmin\Desktop\LCHWC\PostHardware.txt
echo Next step is comparison. Below are the changes from the pre-hardware check to post-hardware check.
echo If all you see is a timestamp, that means no hardware changed:
echo.
echo.
...
findstr /vg:C:\Users\cladmin\Desktop\LCHWC\PreHardware.txt C:\Users\cladmin\Desktop\LCHWC\PostHardware.txt > C:\Users\cladmin\Desktop\LCHWC\Results.txt
type C:\Users\cladmin\Desktop\LCHWC\Results.txt
echo.
...
set /p PreNotes=""
cls
echo %PreNotes%>> C:\Users\cladmin\Desktop\LCHWC\PreHardware.txt
...
::exit script, you can `goto :eof` if you prefer that. Also, if you want to keep the cmd prompt open use `Exit /B`
pause
Exit
::subroutine (defining the label called Empty as called in line 78)
:Empty
