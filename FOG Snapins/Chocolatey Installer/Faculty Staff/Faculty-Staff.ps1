<#
  .SYNOPSIS
    Installs Chocolatey for Faculty & Staff.

  .DESCRIPTION
    This script installs Chocolatey with the
    settings necessary for CEDC Faculty & Staff.
    This includes the Faculty/Staff repo w/ credentials.
  ...
  .Snapin Description
    Installs Chocolatey with the internal faculty/staff repo using encrypted credentials. 
...
choco source add --name="'CEDC Faculty/Staff'" --source="'http://cedc-nc2413-z1:8081/repository/CEDCFS/'" --priority="'0'" -u="$global:Username" -p="$global:Password"
