<#
  ...
  .To-Do 
   ...
   DONE 6-5-23 Add Test-ComputerSecureChannel repair to the options (used to repair domain-trust relationships) https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.management/test-computersecurechannel?view=powershell-5.1
   ...
   script use = ask to restart, maybe more logic and checks than cmd prompt use. But if name longer than allowed, don't do it and error on cmd line
...
{
    Write-Host "Putting hostname $JoinDomain in AD Path ucdenver.pvt>CEAS->$ou1->$ou2.->$ou3->$ou4"
    $JoinNow = Read-Host "If this is the correct, would you like to join now?(Y or N)"
       switch ($JoinNow){
                'Y'{
                ...
                elseif($JoinDomain -like "*-*-*-*" ){Write-Verbose "SERVER DETECTED." #really for renaming host. if $JoinDomain has 3 dashes
                try{Add-Computer -ComputerName $JoinDomain -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=SRVS,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorVariable computerError -WhatIf #-ErrorAction Stop
                  }catch{Write-Error "`n$_"} 
                }
                else{Write-Verbose "LABS OR SERVER NOT DETECTED!" -Verbose
...
if ($PSBoundParameters.ContainsKey('UnjoinDomain'))
{
try{& ${PSScriptRoot}\PsExec.exe \\$Server -accepteula  -u $username -p $password -h -s powershell.exe Enable-PSRemoting -Force;Invoke-command -computername $Server -Credential $Credential -scriptblock {Set-ExectutionPolicy bypass -force}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
else {try{& ${PSScriptRoot}\PsExec.exe \\$Server -accepteula -h -s powershell.exe Enable-PSRemoting -Force;Invoke-command -computername $Server -scriptblock {Set-ExectutionPolicy bypass -force}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}}
exit
}
if ($PSBoundParameters.ContainsKey('RenameHost'))
{
try{& ${PSScriptRoot}\PsExec.exe \\$Server -accepteula  -u $username -p $password -h -s powershell.exe Enable-PSRemoting -Force;Invoke-command -computername $Server -Credential $Credential -scriptblock {Set-ExectutionPolicy bypass -force}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
else {try{& ${PSScriptRoot}\PsExec.exe \\$Server -accepteula -h -s powershell.exe Enable-PSRemoting -Force;Invoke-command -computername $Server -scriptblock {Set-ExectutionPolicy bypass -force}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}}
exit
}
if ($PSBoundParameters.ContainsKey('SecureChannel'))
{ #If LOR = L do this, if R else. Then if test or repair
If ($PSBoundParameters.ContainsKey('Repair'))
{If ($PSBoundParameters.ContainsKey('Remote')){
try{Invoke-Command -ComputerName $SecureChannel -ScriptBlock {Test-ComputerSecureChannel -Credential $Credential -Repair -Verbose}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                                              }
elseif (!$PSBoundParameters.ContainsKey('Remote')){
try{Test-ComputerSecureChannel -Credential $Credential -Repair -Verbose}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                                                  }
}
If ($PSBoundParameters.ContainsKey('Test'))
{If ($PSBoundParameters.ContainsKey('Remote')){
try{Invoke-Command -ComputerName $SecureChannel -ScriptBlock {Test-ComputerSecureChannel -Credential $Credential -Verbose}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                                              }
}
elseif (!$PSBoundParameters.ContainsKey('Remote')){
try{Test-ComputerSecureChannel -Verbose}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
}
If ($PSBoundParameters.ContainsKey('Repair') -and $PSBoundParameters.ContainsKey('Test') )
{If ($PSBoundParameters.ContainsKey('Remote')){
try{Invoke-Command -ComputerName $SecureChannel -ScriptBlock {Test-ComputerSecureChannel -Credential $Credential -Verbose};Invoke-Command -ComputerName $SecureChannel -ScriptBlock {Test-ComputerSecureChannel -Credential $Credential -Repair -Verbose}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                                              }
elseif (!$PSBoundParameters.ContainsKey('Remote')){
try{Test-ComputerSecureChannel -Credential $Credential -Verbose;Test-ComputerSecureChannel -Credential $Credential -Repair -Verbose}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                                                  }
}
exit
}
...
function Show-Menu
{
    ...
    Write-Host "5: Press '5' to repair the trust relationship (SecureChannel)."
...
process {
        ...
        switch ($PSBoundParameters.Keys) {
            ...
            'SecureChannel'{
                    ...
                    If ($RorL -eq 'L'){
                    try{Test-ComputerSecureChannel -Credential $Credential -Verbose;pause;Test-ComputerSecureChannel -Credential $Credential -Repair -Verbose}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                    }
                    elseif ($RorL -eq 'R'){
                    try{Invoke-Command -ComputerName $SecureChannel -ScriptBlock {Test-ComputerSecureChannel -Credential $Credential -Verbose};pause;Invoke-Command -ComputerName $SecureChannel -ScriptBlock {Test-ComputerSecureChannel -Credential $Credential -Repair -Verbose}}catch{Write-Error “`nAn error occurred: $($_.Exception.Message)`n”}
                    }
...
:start do
 {
    ...
    '2'{Write-Host "You chose option #2: Unjoin the local PC from the domain."
        DJT -UnjoinDomain 
    }'3'{'You chose option #3: Rename this host locally.'
    ...
    }'4'{'You chose option #4: Join the domain using the current hostname.'
         #LOGIC for local or remote
         ...
         if ($LocalOrRemote -eq 'L'){ #DJT -JoinDomainL
            ...
            DJT -JoinDomain "$hostname" -ou1 "$1ou" -ou2 "$2ou"
                                    }
    ...
    }'5'{'Domain Trust Relationship Fix'
         DJT -SecureChannel
    }'Q'{exit}
     Default {
