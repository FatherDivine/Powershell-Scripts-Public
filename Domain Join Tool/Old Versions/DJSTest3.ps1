...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer -Credential $Credential -Server "ucdenver.pvt" -Identity $server -ErrorAction SilentlyContinue).Count) {
        Write-Host "#######################################"  
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=BIOE,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
#Import-Module ActiveDirectory
    ...
    Do {
       ...
       } while (Get-ADComputer -Server "ucdenver.pvt" -filter "Name -like '*$CustomHostInc'" -Credential $Credential) #Server = Artemis Domain Controllers 
                                 }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=CIVL,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=CSCI,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=DEAN,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=ECSG,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=ELEC,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=IWKS,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=MECH,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=SRVS,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    if (@(Get-ADComputer $server -ErrorAction SilentlyContinue).Count) {
        ...
        try{
            Add-Computer -DomainName "ucdenver.pvt"  -Credential $Credential -OUPATH "OU=TEST,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction Stop -ErrorVariable computerError 
           }
...
foreach ($server in $serverlist) {
    ...
    Add-Computer  -ComputerName (Get-Content -Path ${PSScriptRoot}\$ComputerListLocation) -Credential $Credential -DomainName "ucdenver.pvt" -OUPATH "OU=$ComputerOULocation,OU=CEAS,DC=ucdenver,DC=pvt" -ErrorAction SilentlyContinue -ErrorVariable computerError
    $computerError
    "One and done"
    pause
    }
  ...
  $RenameHost = Read-Host "What would you like your new hostname to be? (Will keep asking until 15 characters or less)"
      ...
      "To join the domain, we will use the first 4 letters of your current hostname to set the correct OU path. Those first four are $ReadOU."  
