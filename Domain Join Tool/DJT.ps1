...
function Show-Menu
{
    ...
    Write-Host "6: Press '6' to enable PS Remote Services (WinRM/WSMan)"
...
pause;pause
            ...
            elseif ($PSBoundParameters.ContainsKey('RenameHost')){
              ...
              if ($Restart -eq "Y"){
              try{Invoke-Command -Computer $UnjoinDomain -ArgumentList $Credential -ScriptBlock {
...
#function CheckIfHNExistsinAD
    ...
    {'C'{ 
           ...
           $CutLength = $hostname
           $ReadOU = $CutLength.SubString(0,5)
           Write-Host "$ReadOU";pause #Test function
    ...
    }'6'{ 'Enable PS Remote Services (WinRM/WSMan)'
